...
collect_more_data = False # Otherwise just load the data
...
inputs = ["examples/tate-ex0.srql", "examples/tate-ex1.srql","examples/tate-ex1b.srql","examples/tate-ex2.srql","examples/tate-ex2b.srql", "examples/tate-ex3.srql", "examples/tate-ex4.srql", "examples/tate-ex5.srql","examples/tate-ex6.srql",]
...
inputs_names = ["Query 0", "Query 1", "Query 1b", "Query 2", "Query 2b", "Query 3", "Query 4", "Query 5", "Query 6"]
inputs_names_sz = 8
meths = ["s", "c", "[c]",  "[c][s]", "[s][c]"]
...
ax1.set_ylabel("Plan size (log)", fontweight ='bold', size=10)
#ax1.set_ylim([1, 1200])
...
ax2.set_ylabel("Execution cost (log)", fontweight ='bold', size=10)
#ax2.set_ylim([1, 2350])
ax2.set_ylim([150, 2500])
ax2.set_yscale('log')

plt.subplots_adjust(hspace=.0)
plt.savefig('quality-combined-right.pdf', format='pdf')
plt.close(plt.gcf())
